  ...
  reGenerateOTP() {
    ...
    this.http.post(this.appService.API_ENDPOINT +'reGenerateOTP', this.Indata)
      .subscribe(data => {
        ...
        if (data == 1) {
          this.resnt = true;
          this.appService.presentToast("Check your Email, We have re-sent the OTP.", 'bottom',3000);
        }
        else {
          this.resnt = false;
          this.appService.presentToast("Oops, Check your given Email adress.", 'bottom',3000);
        }
      },
        err => console.log(err),
        () => console.log());
