...
export class ChatBoxPage implements OnInit {
    ...
    isMessageSent(input) {
        ...
        if (input == "voice") {
            if(this.isBrowser){
                alert("Voice text will be enabled in devices alone...");
            }
            else{
                let options = {
                    language: 'th-TH'
    ...
    GetOneComunicationChatHis() {
        this.http.get(this.appService.API_ENDPOINT + 'whichChatEnabled', {
            ...
            .subscribe(data => {
                    ...
                    this.receiverProPic = this.appService.mediaServer + data[0].pro_path+"?"+new Date().getSeconds();
                },
                err => console.log(err),
        ...
        if (a == "open") {
            ...
            this.http.post(this.appService.API_ENDPOINT + 'userProfileDetails', this.postData)
                .subscribe(data => {
                        ...
                        this.seeUserDataArray[0].cover_path= this.seeUserDataArray[0].cover_path+"?"+new Date().getSeconds();
                        this.seeUserDataArray[0].pro_path=  this.seeUserDataArray[0].pro_path +"?"+new Date().getSeconds();
